...
const { ensureUploadDir, uploadMiddleware } = require("../utils/storage");
...
router.post(
  ...
  async (req, res) => {
    try {
      ...
      const finalName = `${video._id}-${Date.now()}${ext}`;

      const tenantPath = path.join(UPLOAD_DIR, req.user.tenantId);
      const finalPath = path.join(tenantPath, finalName);

      fs.renameSync(file.path, finalPath);

      video.filename = finalName;
      await video.save();
...
router.get("/stream/:id", async (req, res) => {
  try {
    ...
    const filePath = path.join(UPLOAD_DIR, video.tenantId, video.filename);
...
router.delete("/:id", auth, permit("editor", "admin"), async (req, res) => {
  try {
    ...
    const filePath = path.join(UPLOAD_DIR, v.tenantId, v.filename);
    try {
